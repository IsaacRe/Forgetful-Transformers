...
def consolidate_scissorhands(attn_weights: torch.Tensor):
    c_range = CONFIG.consolidate_range
    f_range = CONFIG.fit_range

    consolidate_attn = attn_weights[...,f_range,c_range]
    max_activations: torch.Tensor = consolidate_attn.cummax(dim=-2).values

    thresholds = torch.quantile(max_activations, CONFIG.consolidate_ratio, dim=-1)
    evict_mask = max_activations < thresholds
    if (evict_mask.sum(dim=-1) != int(consolidate_attn.shape[-1] * CONFIG.consolidate_ratio)).any():
        GLOBALS.vars['failing_attn'] = attn_weights.cpu()
        GLOBALS.vars['failing_mask'] = evict_mask.cpu()
        GLOBALS.vars['failing_thresholds'] = thresholds.cpu()
        raise AssertionError

    attn_weights[...,f_range,c_range][evict_mask] = 0
...
def modified_llama_attn_forward(
    ...
    **kwargs,
    ...
    if (
        ...
        CONFIG.consolidate_method == "scissorhands" and
